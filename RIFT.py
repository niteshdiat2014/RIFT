...
if destdir!='':
    ...
    raise Exception('\n Destination directory identification un-successful')
...
fileaccess = os.path.join(drname1,'Accessdenied.csv')
...
def extfiles():
    dirs=["/root/","/etc/default/","/etc/network/", "/var/log/", "/etc/wpa_supplicant/",
          ...
          "/proc/stat","/proc/diskstats","fdisk -l"]
    os.mkdir(drname1)
    f1=open(fname1,'wa')
    ...
    for i in dirs:
        ...
        if fileverification:
            if accesspermission:
                ...
                internaldir = drname1+internaldir
...
def cmndout():
    ...
    dirs=["ifconfig", "uname -a", "gpio readall", "netstat -l", "netstat -s","netstat -ie", "netstat --statistics --raw",
          ...
          "ss -tl6", "ss -t4 state established", "ss -t4 state time-wait"]
    for i in dirs:
        filei=i.replace(" ", "")
        filei=filei.replace("|", "")
        filei=filei.replace("/", "")
...
def archiving():
    tmarchive = tm
    tmarchive=tmarchive.replace(" ","")
    tmarchive=tmarchive.replace(":","")
    archvname = os.path.expanduser(os.path.join(destdir,tmarchive))
    sourcedir = drname1
